# SCHEMA — VAA-authorized DCA into NEAR Intents

Companion to `spec.md`. Concrete wire formats, interfaces, and state layout.

Anything marked **⚠ UNVERIFIED** must be pinned against the live contract or API
before implementation. See `spec.md` §7.
...
account id *is* an Ed25519 public key.
...
> `predecessor` is an explicit argument on the *view* method, so anyone can compute
...
or any future route — the emitter publishes an *order*, and what varies is only how
...
Packed big-endian. Fixed 70-byte header, then two length-prefixed strings.
Total size `70 + destAssetLen + recipientLen`.

| Offset | Size | Field | Type | Notes |
|---|---|---|---|---|
| 0 | 1 | `version` | u8 | `1`. Reject anything else. |
| 1 | 32 | `orderId` | bytes32 | Route identifier, and the MPC derivation path. Must be unique per (source, destination, recipient). |
| 33 | 32 | `intentsAccount` | bytes32 | The derived `A`, raw. The router re-derives and **rejects on mismatch** — catches client/router config drift loudly instead of stranding funds. |
| 65 | 2 | `maxSlippageBps` | u16 | Floor tolerance, committed at order creation. |
| 67 | 1 | `recipientKind` | u8 | `0` = destination chain (→ `ft_withdraw`), `1` = intents account (→ `transfer`). |
| 68 | 1 | `destAssetLen` | u8 | |
| 69 | D | `destinationAsset` | bytes | UTF-8 intents token id, e.g. `nep141:zec.omft.near`. Use the bare id, **not** 1Click's `1cs_v1:` form. |
| 69+D | 1 | `recipientLen` | u8 | |
| 70+D | R | `recipient` | bytes | UTF-8 destination address. **The value the whole design protects.** |
...
⚠ **UNVERIFIED:** `SignRequest`'s field names on `sign`. The *view* method
...
failed *publish* reproduces the same nonce rather than consuming a new one.
...
Note `payload.recipient` is `intents.near` — the NEP-413 *verifying contract*, not
...
`maxSlippageBps` — all immutable policy. Amounts are *observations*, so they cannot
...
{ "method": "query", "params": {
    "request_type": "call_function", "finality": "final",
    "account_id": "intents.near", "method_name": "mt_batch_balance_of",
...
```json
{ "id": 1, "jsonrpc": "2.0", "method": "quote", "params": [{
...
```json
{ "result": [{ "quote_hash": "…", "amount_in": "…", "amount_out": "…",

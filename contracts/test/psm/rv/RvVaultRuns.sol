...
import {IAaveOracle, IPool, IPoolAddressesProvider} from "../../../src/psm/interfaces/IAave.sol";
...
contract RvVaultRuns is MessageReceiver, AccessControlUpgradeable, IHollarBaseVault {
    ...
    /// @dev Dead-run index. For a run of cancelled slots [a, b]: skipTo[a] = b + 1 and
    ///      runStart[b] = a. Only `cancelQueuedRedemption` ever writes these, so nothing on the
    ///      deposit or payment path pays for them.
    mapping(uint256 => uint256) public skipTo;
    mapping(uint256 => uint256) public runStart;
    ...
    function queueEntryOf(address recipient) external view returns (bool found, uint256 index, uint256 position) {
        uint256 seen;
        for (uint256 i = queueHead; i < queueTail;) {
            if (queue[i].amount == 0) {
                i = _runEnd(i);
                continue;
            }
    ...
    function _advanceHead() private {
        ...
        uint256 hops;
        while (head < tail && queue[head].amount == 0) {
            head = _runEnd(head);
            hops++;
            require(hops <= 2, "RUN INDEX BROKEN"); // review harness: the bound is the claim
        }
        queueHead = head;
    }

    function _runEnd(uint256 i) private view returns (uint256) {
        uint256 e = skipTo[i];
        return e == 0 ? i + 1 : e;
    }

    /// @dev O(1) removal of an interior slot: merge with the dead run on either side.
    function _retire(uint256 index) private {
        uint256 left = index;
        uint256 right = index + 1;
        uint256 head = queueHead;
        if (index > head && queue[index - 1].amount == 0) {
            uint256 rs = runStart[index - 1];
            left = rs == 0 ? index - 1 : rs;
            if (left < head) left = index; // never merge across the head
        }
        if (right < queueTail && queue[right].amount == 0) right = _runEnd(right);
        skipTo[left] = right;
        runStart[right - 1] = left;
    }
    ...
    function _liveHead() private view returns (bool found, uint256 index) {
        ...
        uint256 tail = queueTail;
        uint256 hops;
        while (i < tail && queue[i].amount == 0) {
            i = _runEnd(i);
            hops++;
            require(hops <= 2, "RUN INDEX BROKEN");
        }
        return i < tail ? (true, i) : (false, 0);
    }

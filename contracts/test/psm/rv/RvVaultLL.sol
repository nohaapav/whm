...
contract RvVaultLL is MessageReceiver, AccessControlUpgradeable, IHollarBaseVault {
    ...
    /// @dev The reviewed proposal: `uint64 next` packed into recipient's spare 12 bytes.
    ///      next == 0 means "implicit successor" (index+1) so an enqueue writes no extra slot.
    struct CreditLL {
        address recipient;
        uint64 next;
        uint256 amount;
    ...
    mapping(uint256 => CreditLL) public queue;
    uint256 public queueHead;
    uint256 public queueTail;

    error BadHint(uint256 prev, uint256 index);
    ...
    function cancelQueuedRedemption(uint256 index) external payable returns (uint64 sequence) {
        return cancelQueuedRedemption(index, index == 0 ? 0 : index - 1);
    }

    /// @param prev caller-supplied hint, validated on-chain as the proposal specifies.
    function cancelQueuedRedemption(uint256 index, uint256 prev) public payable returns (uint64 sequence) {
        CreditLL memory credit = queue[index];
        if (credit.amount == 0) revert NotQueued(index);
        if (credit.recipient != msg.sender) revert NotYourCredit(index, credit.recipient);

        if (index == queueHead) {
            queueHead = _nextOf(index);
        } else {
            if (_nextOf(prev) != index) revert BadHint(prev, index);
            queue[prev].next = uint64(_nextOf(index));
        }
    ...
    function queueEntryOf(address recipient) external view returns (bool found, uint256 index, uint256 position) {
        uint256 seen;
        for (uint256 i = queueHead; i < queueTail; i = _nextOf(i)) {
    ...
    function _enqueue(address recipient, uint256 amount, uint256 gross) private {
        if (amount == 0) return;
        queue[queueTail] = CreditLL({recipient: recipient, next: 0, amount: amount, gross: gross});
        queueTail++;
    ...
    function _advanceHead() private {
        // No walk remains: the list is supposed to contain only live entries.
    }

    function _nextOf(uint256 i) internal view returns (uint256) {
        uint64 n = queue[i].next;
    ...
    function _liveHead() private view returns (bool found, uint256 index) {
        uint256 h = queueHead;
        return h < queueTail ? (true, h) : (false, 0);
    }
